...
export const EstimateBuilder = () => {
  ...
  const { createEstimate, isSaving } = useEstimates();
  ...
  const handleSave = async () => {
    // Filter out empty rows where they didn't type anything
    ...
    const result = await createEstimate(validItems, subtotal);

    if (result.success) {
      alert('Estimate Saved Successfully!');
      // We will eventually route them to the beautiful invoice view here:
      // router.push(`/dashboard/estimates/${result.jobId}`);
      router.push('/dashboard'); // Kick them back to the dashboard for now
    } else {
    ...
    <Card className="border-slate-200 p-6 shadow-sm md:p-8">
      ...
      <div className="mt-8 flex justify-end">
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="flex items-center gap-2 rounded-xl bg-slate-900 px-8 py-3 font-bold text-white shadow-lg transition-all hover:-translate-y-0.5 hover:bg-slate-800 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
        >
